...
export class Inventory{
        ...
        for(let i=0;i<9;i++){
            for(let d=0;d<6;d++){
                    if(item.isStackable()==true) {
                        for(let k=0;k<9;k++){
                            for(let v=0;v<6;v++){
                                if(this.#itemMas[k][v].assignedItem!=null){
                                    if(this.#itemMas[k][v].assignedItem.name==item.name) {
                                        if(this.#itemMas[k][v].Stack + 1 <= this.#itemMas[k][v].maxStack) {
                                            this.#itemMas[k][v].Stack+=1;
                                        return;
                                    }
                                }
                                }
    ...
    setPopupInfo(){
        ...
        let charsDiv = document.getElementsByClassName("item-chars")[0];
